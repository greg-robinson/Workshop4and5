...
export default class Comment extends React.Component {
  ...
  handleLikeClick(clickEvent) {
    ...
    if (clickEvent.button === 0) {
      ...
      if (this.didUserLike()) {
        ...
        unlikeComment(this.state.parent, this.state.index, 4, callbackFunction);
      } else {
        // User clicked 'like' button.
        likeComment(this.state.parent, this.state.index, 4, callbackFunction);
      }
    }
  }
  ...
  didUserLike() {
    ...
    // current user.
    for (var i = 0; i < this.state.commentLikeCounter.length; i++) {
      if (commentLikeCounter[i]._id === 4) {
        liked = true;
        break;
      }
    }
        ...
        <div className="media-body">
          <Link to={"/profile/" + this.props.author._id}>{this.props.author.fullName}</Link> {this.props.children}
            <a href="#" onClick={(e) => this.handleLikeClick(e)}>
              <br />
                <span className="glyphicon glyphicon-thumbs-up"></span> {likeButtonText} ({this.state.commentLikeCounter.length})
            </a>
            &ensp; {unixTimeToString(this.props.postDate)}
        </div>
      </div>
    )
  }
